...
Think harder. All-in-one ClaudeKit guide. Run the script and enhance output for user.
...
**3. Offer Follow-Up** After showing results, offer relevant next actions:
...
**User:** `/ck-help` **You:** Here's an overview of ClaudeKit commands: [script output] What would you like to explore?

**User:** `/ck-help fix` **You:** Here's the fixing workflow: [script output] Which issue are you dealing with?

**User:** `/ck-help add login page` **You:** For building a login page, I'd recommend: [script output] Try
`/cook add login page` to implement directly, or `/plan add login page` then `/code` for explicit planning first.
