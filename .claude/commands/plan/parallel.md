...
Think strategically about parallelization. Activate `planning` skill.
...
1. Create a directory named `plans/YYYYMMDD-HHmm-plan-name` (eg.
...
5. Main agent gathers all research and scout report filepaths, and pass them to `planner` subagent with the prompt to
   create a parallel-optimized implementation plan.
...
- Save the overview access point at `plans/YYYYMMDD-HHmm-plan-name/plan.md`. Keep it generic, under 80 lines, and list
  each implementation phase with status, progress, parallelization group, and links to phase files.
...
**IMPORTANT:** Do not start implementing. **IMPORTANT:** Each phase MUST have exclusive file ownership - no file can be
modified by multiple phases.
