...
Aggregation processes documents through multiple stages. Each stage transforms documents and passes results to next
stage.
...
db.orders.aggregate([
  {
    $match: {
      $and: [{ orderDate: { $gte: startDate } }, { status: { $in: ['completed', 'shipped'] } }],
    },
  },
]);
```
...
const page = 2;
const pageSize = 20;
db.orders.aggregate([{ $sort: { orderDate: -1 } }, { $skip: (page - 1) * pageSize }, { $limit: pageSize }]);
```
...
db.orders.aggregate([
  {
    $lookup: {
      from: 'products',
      let: { items: '$items' },
      pipeline: [{ $match: { $expr: { $in: ['$_id', '$$items.productId'] } } }, { $project: { name: 1, price: 1 } }],
      as: 'productDetails',
    },
  },
]);
...
db.products.aggregate([
  {
    $project: {
      ...
      margin: { $multiply: [{ $divide: [{ $subtract: ['$price', '$cost'] }, '$price'] }, 100] },
    },
  },
]);
...
db.users.aggregate([
  {
    $project: {
      ...
      email: { $toLower: '$email' },
      initials: { $concat: [{ $substr: ['$firstName', 0, 1] }, { $substr: ['$lastName', 0, 1] }] },
    },
  },
]);
...
db.events.aggregate([
  {
    $project: {
      ...
      ageInDays: { $divide: [{ $subtract: [new Date(), '$createdAt'] }, 1000 * 60 * 60 * 24] },
    },
  },
]);
```
...
db.products.aggregate([
  { $match: { category: 'electronics' } },
  {
    $facet: {
      ...
      topBrands: [{ $group: { _id: '$brand', count: { $sum: 1 } } }, { $sort: { count: -1 } }, { $limit: 5 }],
