...
db.users.replaceOne({ _id: ObjectId('...') }, { name: 'Alice', email: 'alice@example.com', age: 31 });
```
...
db.posts.updateOne({ _id: postId, 'comments.author': 'Alice' }, { $set: { 'comments.$.text': 'Updated comment' } });
...
// Return new document
db.users.findOneAndUpdate({ email: 'alice@example.com' }, { $set: { status: 'active' } }, { returnNewDocument: true });

// Upsert and return new
db.counters.findOneAndUpdate({ _id: 'sequence' }, { $inc: { value: 1 } }, { upsert: true, returnNewDocument: true });
```
