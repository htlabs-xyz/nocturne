...
Indexes improve query performance by allowing MongoDB to scan fewer documents. Without indexes, MongoDB performs
collection scans (reads every document).
...
// Specific paths only
db.products.createIndex({ '$**': 1 }, { wildcardProjection: { 'attributes.color': 1, 'attributes.size': 1 } });
```
