...
Full working examples combining all modern patterns: React.FC, lazy loading, Suspense, useSuspenseQuery, styling,
  ...
  return useMutation({
    ...
    onMutate: async (userId) => {
      ...
      queryClient.setQueryData<User[]>(['users'], (old) => {
        return old?.map((user) => (user.id === userId ? { ...user, active: !user.active } : user)) || [];
      });
