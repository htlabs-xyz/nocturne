...
**Use:** New codebase, architecture review, complete LLM context, planning **Tips:** Remove comments, use markdown,
check token limits, review before sharing
...
**Use:** Feature analysis, debugging specific areas, targeted refactoring **Tips:** Include related files only, stay
within token limits, use XML for AI
...
**Workflow:** Package library → enable security checks → review vulnerabilities → check suspicious patterns → AI
analysis **Check for:** API keys, hardcoded credentials, network calls, obfuscation, malicious patterns
...
**Use:** API docs, architecture docs, user guides, onboarding **Tips:** Include existing docs, include source, use
markdown
...
**Include:** Routes, controllers, schemas, middleware **Workflow:** Package → AI → OpenAPI/Swagger → endpoint docs →
examples
...
**Problem:** Exceeds LLM token limits **Fix:**
...
**Problem:** Expected files not included **Debug:**
...
**Problem:** Security scanner flags secrets **Actions:** Review files → add to `.repomixignore` → remove sensitive data
→ use env vars
...
**Problem:** Slow on large repo **Optimize:**
...
**Problem:** Cannot access remote repo **Fix:**
