import { useState } from 'react';

interface HeaderProps {
  accountName?: string;
  network?: string;
  onAccountClick?: () => void;
  onSettingsClick?: () => void;
}

export function Header({
  accountName = 'Account 1',
  network = 'Devnet',
  onAccountClick,
  onSettingsClick,
}: HeaderProps) {
  const [showNetworkBadge] = useState(true);

  return (
    <header className="flex items-center justify-between px-4 py-3 bg-midnight-800 border-b border-midnight-600">
      <div className="flex items-center gap-2">
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-accent-purple to-indigo-600 flex items-center justify-center">
          <span className="text-sm font-bold text-white">M</span>
        </div>
        <span className="font-semibold text-white">Midnight</span>
      </div>

      <div className="flex items-center gap-3">
        {showNetworkBadge && (
          <span className="px-2 py-0.5 text-xs font-medium bg-midnight-600 text-accent-green rounded-full">
            {network}
          </span>
        )}

        <button
          onClick={onAccountClick}
          className="flex items-center gap-1 px-2 py-1 rounded-lg hover:bg-midnight-700 transition-colors"
        >
          <div className="w-5 h-5 rounded-full bg-gradient-to-br from-purple-400 to-pink-500" />
          <span className="text-sm text-text-secondary">{accountName}</span>
          <svg className="w-3 h-3 text-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
          </svg>
        </button>

        <button
          onClick={onSettingsClick}
          className="p-1.5 rounded-lg hover:bg-midnight-700 transition-colors text-text-secondary hover:text-white"
        >
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
          </svg>
        </button>
      </div>
    </header>
  );
}
