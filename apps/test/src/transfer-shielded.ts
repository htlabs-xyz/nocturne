...
export async function transferShielded(amount: bigint = 1_000_000n) {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║         Midnight Wallet SDK - Shielded Transfer            ║');
  console.log('╚════════════════════════════════════════════════════════════╝\n');
  ...
  const { facade: receiverFacade } = createWalletFacade(receiverKeys, networkId);
  ...
  if (shieldedBalance < amount) {
    console.log(`⚠️ Insufficient Shielded balance. Have: ${formatAmount(shieldedBalance)}, Need: ${formatAmount(amount)}`);
  ...
  console.log(`Receiver shielded address: ${receiverShieldedAddress.slice(0, 30)}...\n`);

  console.log('┌────────────────────────────────────────────────────────────┐');
  console.log('│ Creating Shielded Transfer Transaction                     │');
  console.log('└────────────────────────────────────────────────────────────┘\n');
  ...
  console.log(`✓ Shielded transfer: ${formatAmount(amount)} → ${receiverShieldedAddress.slice(0, 20)}...`);
  ...
  console.log(`Receiver Shielded balance: ${formatAmount(receiverShieldedBalance)}\n`);

  const success = receiverShieldedBalance >= amount;
  if (success) {
  ...
  console.log('\n╔════════════════════════════════════════════════════════════╗');
  console.log(`║          SHIELDED TRANSFER ${success ? 'COMPLETE ✅' : 'FAILED ❌'}                    ║`);
  console.log('╚════════════════════════════════════════════════════════════╝\n');
